...
C     ..
   ...
   20 VARY(1) = EXP(PI**2/LOG(U))
      ...
      DOUBLE COMPLEX
C  -------------------------
     +  FUNCTION WTHETA(UARY,VARY,DLAM,IU,U,W)
C  -------------------------
...
C     ..
   ...
   30 WTHETA = EXP(-WT**2/ (4.D0*PI*DLAM))*WTHETA/SQRT(DLAM)
      ...
      DOUBLE COMPLEX
C   --------------------------------------------
     +  FUNCTION WPROD(UARY,VARY,DLAM,IU,W,M,N,U,W0,W1,ALFA0,ALFA1)
C   --------------------------------------------
...
C     ..
      WSUM = (0.D0,0.D0)
      DO 10 K = 1,M
          WTH = LOG(WTHETA(UARY,VARY,DLAM,IU,U,W/ (U*W0(K))))
          WSUM = WSUM + (ALFA0(K)-1.D0)*WTH
   10 CONTINUE
      DO 20 K = 1,N
          WTH = LOG(WTHETA(UARY,VARY,DLAM,IU,U,U*W/W1(K)))
...
C     .. Scalar Arguments ..
      INTEGER, INTENT(IN) :: M,N,NPTQ
C     ..
C     .. Array Arguments ..
      DOUBLE PRECISION, INTENT(IN) :: ALFA0(M),ALFA1(N)
      DOUBLE PRECISION, INTENT(OUT) :: QWORK(NPTQ*(2*(M+N)+3))
C     ..
C     .. Local Scalars ..
...
C   COMPUTE NODES & WEIGHTS FOR PURE GAUSSIAN QUADRATURE:
      ...
      DOUBLE COMPLEX
C   ------------------------------------------------------------
     +  FUNCTION WQSUM(UARY,VARY,DLAM,IU,WA,PHIA,KWA,IC,WB,PHIB,RADIUS,M
     +     ,N,U,W0,W1,ALFA0,ALFA1,NPTQ,QWORK,LINEARC)
C   ------------------------------------------------------------
...
C   INTEGRATE ALONG A LINE SEGMENT:
     ...
     +            WPROD(UARY,VARY,DLAM,IU,W,M,N,U,W0,W1,ALFA0,ALFA1)
   10 CONTINUE
      WQSUM = WQSUM*WH
      RETURN
C
C   INTEGRATE ALONG A CIRCULAR ARC:
   ...
   30 CONTINUE
      ...
      DOUBLE COMPLEX
C   -------------------------------------------------------------
     +     FUNCTION WQUAD1(UARY,VARY,DLAM,IU,WA,PHIA,KWA,IC,WB,PHIB,
     +        RADIUS,M,N,U,W0,W1,ALFA0,ALFA1,NPTQ,QWORK,LINEARC)
C   -------------------------------------------------------------
...
C   STEP1:ONE-SIDED G-J QUADRATURE FOR LEFT ENDPT WA:
      ...
      WAA = WA + R* (WB-WA)
      WQUAD1 = WQSUM(UARY,VARY,DLAM,IU,WA,0.D0,KWA,IC,WAA,0.D0,0.D0,M,N,
     +               U,W0,W1,ALFA0,ALFA1,NPTQ,QWORK,LINEARC)
C
C   STEP2:ADJOIN INTERVALS OF PURE GAUSS QUADRATURE IF NECESSARY:
   20 IF (R.EQ.1.D0) RETURN
      ...
      WBB = WAA + R* (WB-WAA)
      WQUAD1 = WQUAD1 + WQSUM(UARY,VARY,DLAM,IU,WAA,0.D0,0,2,
     +         WBB,0.D0,0.D0,M,N,U,W0,W1,ALFA0,ALFA1,NPTQ,QWORK,LINEARC)
      WAA = WBB
      GO TO 20
*
...
*   STEP1:ONE-SIDED G-J QUADRATURE FOR LEFT ENDPT WA:
   30 R = MIN(1.D0,DIST(M,N,W0,W1,WA,KWA,IC)/ABS(WB-WA))
      ...
      WQUAD1 = WQSUM(UARY,VARY,DLAM,IU,WA,PHIA,KWA,IC,WAA,PHAA,
     +            RADIUS,M,N,U,W0,W1,ALFA0,ALFA1,NPTQ,QWORK,LINEARC)
C
C   STEP2:ADJOIN INTERVALS OF PURE GAUSS QUADRATURE IF NECESSARY:
   40 IF (R.EQ.1.D0) RETURN
      ...
      WQUAD1 = WQUAD1 + WQSUM(UARY,VARY,DLAM,IU,WAA,PHAA,0,2,WBB,PHBB,
     +               RADIUS,M,N,U,W0,W1,ALFA0,ALFA1,NPTQ,QWORK,LINEARC)
      PHAA = PHBB
      WAA = WBB
      GO TO 40
      ...
      DOUBLE COMPLEX
C   ---------------------------------------------------------------
     +     FUNCTION WQUAD(UARY,VARY,DLAM,IU,WA,PHIA,KWA,ICA,WB,PHIB,KWB,
     +       ICB,RADIUS,M,N,U,W0,W1,ALFA0,ALFA1,NPTQ,QWORK,LINEARC,IEVL)
C   ---------------------------------------------------------------
...
C   COMPOUND GAUSS-JACOBI PROCESS ACCORDING TO ONE-QUATER RULE:
      WQA = WQUAD1(UARY,VARY,DLAM,IU,WA,PHIA,KWA,ICA,WMIDA,PHMIDA,
     +            RADIUS,M,N,U,W0,W1,ALFA0,ALFA1,NPTQ,QWORK,LINEARC) -
     +     WQUAD1(UARY,VARY,DLAM,IU,WMID,PHMID,0,2,WMIDA,PHMIDA,
     +                RADIUS,M,N,U,W0,W1,ALFA0,ALFA1,NPTQ,QWORK,LINEARC)
      WQB = WQUAD1(UARY,VARY,DLAM,IU,WB,PHIB,KWB,ICB,WMIDB,PHMIDB,
     +              RADIUS,M,N,U,W0,W1,ALFA0,ALFA1,NPTQ,QWORK,LINEARC) -
     +     WQUAD1(UARY,VARY,DLAM,IU,WMID,PHMID,0,2,WMIDB,PHMIDB,
     +             RADIUS,M,N,U,W0,W1,ALFA0,ALFA1,NPTQ,QWORK,LINEARC)
      WQUAD = WQA - WQB
      RETURN
...
C     ..
   ...
   40 CONTINUE
      ...
      END
C    --------------------------------------
      SUBROUTINE DSCFUN(ISPRT,ICOUNT,UARY,VARY,DLAM,IU,M,N,NPTQ,W0,W1,Z0
     +     ,Z1,ALFA0,ALFA1,PHI0,PHI1,QWORK,NDIM,X,FVAL,IFLAG)
C    --------------------------------------
...
C  TWO EQUATIONS TO ELIMINATE POSSIBLE ROTATION OF THE INNER POLYGON:
      WIN1 = Z1(1) - Z1(N) - C*WQUAD(UARY,VARY,DLAM,IU,W1(N),PHI1(N),N
     +     ,1,W1(1),PHI1(1),1,1,U,M,N,U,W0
     +     ,W1,ALFA0,ALFA1,NPTQ,QWORK,LINEARC,2)
      FVAL(1) = DIMAG(ZI*WIN1)
      FVAL(2) = DIMAG(WIN1)
C
C  N-1 SIDE LENGTH CONDITIONS FOR THE INNER POLYGON:
      DO 10 I = 1,N - 1
         WINT1 = WQUAD(UARY,VARY,DLAM,IU,W1(I),PHI1(I),I,1,W1(I+1)
     +  ,PHI1(I+1),I+1,1,U,M,N,U,W0,W1,ALFA0,ALFA1,NPTQ,QWORK,LINEARC,2)
...
C  COMBINATION OF TWO DIFFERENT PATHS WILL BE USED INSTEAD:
      WX = DCMPLX(U,0.D0)
      WLINE = WQUAD(UARY,VARY,DLAM,IU,W0(M),0.D0,M,0,WX,0.D0,0,2,0.D0
     +              ,M,N,U,W0,W1,ALFA0,ALFA1,NPTQ,QWORK,0,2)
      IF (PHI1(N).LE.0.D0) THEN
         WARC = WQUAD(UARY,VARY,DLAM,IU,W1(N),PHI1(N),N,1,WX,0.D0,0,2
     +                ,U,M,N,U,W0,W1,ALFA0,ALFA1,NPTQ,QWORK,1,2)
          WIN2 = WLINE - WARC

      ELSE
         WARC = WQUAD(UARY,VARY,DLAM,IU,WX,0.D0,0,2,W1(N),PHI1(N),N,1
     +                ,U,M,N,U,W0,W1,ALFA0,ALFA1,NPTQ,QWORK,1,2)
          WIN2 = WLINE + WARC
      END IF
 ...
 20   WIN2 = WQUAD(UARY,VARY,DLAM,IU,W0(M),0.D0,M,0,W1(N),0.D0
     +         ,N,1,0.D0,M,N,U,W0,W1,ALFA0,ALFA1,NPTQ,QWORK,0,2)
...
C  TWO EQUATIONS TO ELIMINATE POSSIBLE ROTATION OF THE OUTER POLYGON:
      WIN3 = Z0(1) - Z0(M) - C*WQUAD(UARY,VARY,DLAM,IU,W0(M),PHI0(M),M
     +     ,0,W0(1),PHI0(1),1,0,1.D0,M,N,U,W0,W1
     +     ,ALFA0,ALFA1,NPTQ,QWORK,LINEARC,2)
...
C  M-3 SIDE LENGTH CONDITIONS OF THE OUTER POLYGON:
      DO 50 J = 1,M - 3
         WINT2 = WQUAD(UARY,VARY,DLAM,IU,W0(J),PHI0(J),J,0,W0(J+1)
     +        ,PHI0(J+1),J+1,0,1.D0,M,N,U,W0,W1
     +        ,ALFA0,ALFA1,NPTQ,QWORK,LINEARC,2)
...
C  TWO LENGTH CONDITIONS WILL BE REPLACED BY A COMPLEX INTEGRAL:
   70 DO 100 K = 1,NSHAPE - 1
          ...
          DO 80 J = IND(K) + 1,IND(K+1) - 2
             WINT3 = WQUAD(UARY,VARY,DLAM,IU,W0(J),PHI0(J),J,0,W0(J+1)
     +        ,PHI0(J+1),J+1,0,
...
C  THAT IT IS NUMERICALLY  MORE EFFICIENT THAN A SINGLE LINE PATH:
          ...
          WLINE1 = WQUAD(UARY,VARY,DLAM,IU,WA,0.D0,IND(K+1)-1,0,WAI
     +         ,0.D0,0,2,0.D0,M,N,U,
     +             W0,W1,ALFA0,ALFA1,NPTQ,QWORK,0,2)
          WLINE2 = WQUAD(UARY,VARY,DLAM,IU,WB,0.D0,IND(K+1)+1,0,WBI
     +         ,0.D0,0,2,0.D0,M,N,U,
     +             W0,W1,ALFA0,ALFA1,NPTQ,QWORK,0,2)
          WCIRCLE = WQUAD(UARY,VARY,DLAM,IU,WAI,PHIA,0,2,WBI,PHIB,0,2
     +         ,RADIUS,M,N,U,W0,W1,
...
C  CALCULATE THE MAXIMUM-NORM OF THE FUNCTION FVAL:
 ...
 9020 FORMAT (2X,I5,6X,E10.4)
      END
C  -----------------------------------------------------------------
      SUBROUTINE DSCSOLV(TOL,IGUESS,U,C,W0,W1,PHI0,PHI1,Z0,Z1,ALFA0,
     +     ALFA1,QWORK,ISHAPE,LINEARC,UARY,VARY,DLAM,IU,
     +     ISPRT,ICOUNT,M,N,NPTQ)
C  -----------------------------------------------------------------
...
C     ..
C     .. Array Arguments ..
      DOUBLE COMPLEX, INTENT(OUT) :: W0(M),W1(N)
      DOUBLE PRECISION, INTENT(OUT) :: PHI0(M),PHI1(N)
     +     ,UARY(8),VARY(3),DLAM
      INTEGER, INTENT(OUT) :: IU, ISPRT, ICOUNT
      DOUBLE COMPLEX, INTENT(IN) :: Z0(M),Z1(N)
      DOUBLE PRECISION ALFA0(M),ALFA1(N),
     +                 QWORK(NPTQ* (2* (N+M)+3))
C     ..
C     .. Scalars in Common ..
      DOUBLE COMPLEX C2
      DOUBLE PRECISION U2
      INTEGER ISHAPE2,LINEARC2,M2,N2,NPTQ2,NSHAPE
C     ..
C     .. Arrays in Common ..
      INTEGER IND(20)
C     ..
...
C  SOLVE NONLINEAR SYSTEM WITH HYBRD:
     ...
     +     ALFA0,ALFA1,PHI0,PHI1,QWORK,DSCFUN,NM,X,FVAL,TOL,MAXFUN,NM
     +     ,NM,DSTEP,DIAG,1,FACTOR,-1,INFO,NFEV,FJAC,42,QW(1),903,
...
C   ----------------------------------------------------------
     ...
     +                PHI1,NPTQ,QWORK,IOPT,UARY,VARY,DLAM,IU)
C   ----------------------------------------------------------
...
C   INTEGRATION FROM WA TO WB ON A LINE SEGMENT:
      ...
      WINT1 = WQUAD(UARY,VARY,DLAM,IU,WA,0.D0,KNEAR,INEAR
     +     ,WB,0.D0,0,2,0.D0,M,N,U,W0,W1,
...
C   INTEGRATION FROM WB TO WW ON A CIRCULAR ARC:
   20 IF (ABS(WB-WW0).LE.1.D-11) THEN
     ...
     +    ABS(PHIWB-PHIWW0)) PHIWW0 = PHIWW0 - 2.D0*PI
      IF (ABS(WB-WA).LE.1.D-11) THEN
         WINT2 = WQUAD(UARY,VARY,DLAM,IU,WB,PHIWB,KNEAR,INEAR
     +        ,WW0,PHIWW0,KWW,IC,DWW0,M,N,
     +            U,W0,W1,ALFA0,ALFA1,NPTQ,QWORK,1,1)
          GO TO 30

      END IF

      WINT2 = WQUAD(UARY,VARY,DLAM,IU,WB,PHIWB,0,2,WW0,PHIWW0,0,2
     +     ,DWW0,M,N,U,W0,W1,ALFA0,
...
C   A COMBINATION OF A CIRCULAR ARC AND LINE SEGMENT(S):
   30 ZDSC = ZA + C* (WINT1+WINT2)
      IF (IBD.EQ.2) THEN
         ZDSC = ZDSC + C*WQUAD(UARY,VARY,DLAM,IU,WW0,0.D0,0,2,WW
     +        ,0.D0,KWW,IC,0.D0,M,N,U,
     +           W0,W1,ALFA0,ALFA1,NPTQ,QWORK,0,1)
      END IF

      RETURN

 40   ZDSC = ZA + C*WQUAD(UARY,VARY,DLAM,IU,WA,0.D0,KNEAR
     +     ,INEAR,WW,0.D0,KWW,IC,0.D0,M,N,U,
     +       W0,W1,ALFA0,ALFA1,NPTQ,QWORK,0,1)
      RETURN
...
C   ----------------------------------------------------
     ...
     +                QWORK,EPS,IOPT,UARY,VARY,DLAM,IU)
C   ----------------------------------------------------
...
C  4.REFINE THE SOLUTION BY NEWTON'S ITERATION:
     ...
     +          PHI1,NPTQ,QWORK,IOPT,UARY,VARY,DLAM,IU)
          WI = WI + WFN/ (C*WPROD(UARY,VARY,DLAM,IU,
     +                            WI,M,N,U,W0,W1,ALFA0,ALFA1))
          IF (ABS(WI).GT.1.D0) WI = WI/ (ABS(WI)+ABS(1.D0-ABS(WI)))
          IF (ABS(WI).LT.U) WI = U*WI* (ABS(WI)+ABS(U-ABS(WI)))/ABS(WI)
...
C
C     .. Scalar Arguments ..
      INTEGER, INTENT(IN) :: I01,MN
C     ..
C     .. Array Arguments ..
      DOUBLE COMPLEX, INTENT(IN) :: Z01(MN)
      DOUBLE PRECISION, INTENT(OUT) :: ALFA01(MN)
C     ..
C     .. Local Scalars ..
      DOUBLE PRECISION PI
      INTEGER K,KL,KR
...
C---------------------------------------------------------------------
C---------------------------------------------------------------------
      SUBROUTINE HYBRD(ISPRT,ICOUNT,UARY,VARY,DLAM,IU,MD,ND,NPTQ,W0,W1
     +     ,Z0,Z1,ALFA0,ALFA1,PHI0,PHI1,QWORK,FCN,N,X,FVEC,XTOL,MAXFEV
     +     ,ML,MU,EPSFCN,DIAG,MODE,FACTOR,NPRINT,INFO,NFEV,FJAC,LDFJAC,R
...
C     **********
C     DEBUG
C      DOUBLE PRECISION UARY(8), VARY(3), DLAM
C      INTEGER IU
C      COMMON /PARAM4/UARY,VARY,DLAM,IU
C      COMMON /PARAM5/ISPRT,ICOUNT
C     .. Scalar Arguments ..
...
C     .. Array Arguments ..
     ...
     +     PHI0(MD), PHI1(ND), QWORK(NPTQ* (2* (ND+MD)+3)),UARY(8),
     +     VARY(3),DLAM
...
C
      IFLAG = 1
      CALL FCN(ISPRT,ICOUNT,UARY,VARY,DLAM,IU,MD,ND,NPTQ,W0,W1,Z0,Z1
     +     ,ALFA0,ALFA1,PHI0,PHI1,QWORK,N,X,FVEC,IFLAG)
      NFEV = 1
...
C
      IFLAG = 2
      CALL FDJAC1(ISPRT,ICOUNT,UARY,VARY,DLAM,IU,MD,ND,NPTQ,W0,W1,Z0,Z1
     +     ,ALFA0,ALFA1,PHI0,PHI1,QWORK,FCN,N,X,FVEC,FJAC,LDFJAC,IFLAG
...
C
      ...
      IFLAG = 0
      IF (MOD(ITER-1,NPRINT).EQ.0) CALL FCN(ISPRT,ICOUNT,UARY,VARY,DLAM,
     +     IU,MD,ND,NPTQ,W0,W1,Z0,Z1,ALFA0,ALFA1,PHI0,PHI1
     +     ,QWORK,N,X,FVEC,IFLAG)
...
C
      IFLAG = 1
      CALL FCN(ISPRT,ICOUNT,UARY,VARY,DLAM,IU,MD,ND,NPTQ,W0,W1,Z0,Z1
     +     ,ALFA0,ALFA1,PHI0,PHI1,QWORK,N,WA2,WA4,IFLAG)
      NFEV = NFEV + 1
      IF (IFLAG.LT.0) GO TO 300
      FNORM1 = ENORM(N,WA4)
C
...
C
      ...
      IFLAG = 0
      IF (NPRINT.GT.0) CALL FCN(ISPRT,ICOUNT,UARY,VARY,DLAM,IU,MD,ND
     +     ,NPTQ,W0,W1,Z0,Z1,ALFA0,ALFA1,PHI0,PHI1,QWORK,N,X,FVEC,IFLAG)
      RETURN
C
...
C
      ...
      SUBROUTINE FDJAC1(ISPRT,ICOUNT,UARY,VARY,DLAM,IU,MD,ND,NPTQ,W0
     +     ,W1,Z0,Z1,ALFA0,ALFA1,PHI0,
...
C     .. Scalar Arguments ..
      ...
      INTEGER IFLAG,LDFJAC,ML,MU,N,MD,ND,NPTQ,IU,ISPRT,ICOUNT
C     ..
C     .. Array Arguments ..
...
C     ..
C     .. Local Scalars ..
      DOUBLE PRECISION EPS,EPSMCH,H,TEMP,ZERO,UARY(8),VARY(3),DLAM
      INTEGER I,J,K,MSUM
C     ..
C     .. External Functions ..
...
C
      DO 20 J = 1,N
          ...
          IF (H.EQ.ZERO) H = EPS
          X(J) = TEMP + H
          CALL FCN(ISPRT,ICOUNT,UARY,VARY,DLAM,IU,MD,ND,NPTQ,W0,W1,Z0
     +         ,Z1,ALFA0,ALFA1,PHI0,PHI1,QWORK,N,X,WA1,IFLAG)
