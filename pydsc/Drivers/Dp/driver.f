...
C     ..
C     .. Common blocks ..
C      COMMON /PARAM4/UARY,VARY,DLAM,IU
C     ..
      PI = ACOS(-1.D0)
C
...
C   SOLVE THE ACCESSORY PARAMETER PROBLEM:
     ...
     +     LINEARC,ISPRT,ICOUNT,UARY,VARY,DLAM,IU)
C
...
C   COMPUTE DATA FOR THETA-FUNCTION AND TEST THE ACCURACY:
      ...
      CALL DSCTEST(UARY,VARY,DLAM,IU,M,N,U,C,W0
     +     ,W1,Z0,Z1,ALFA0,ALFA1,NPTQ,QWORK)
C
C  FOLLOWING PARAGRAPH IS FOR TESTING INVERSE EVALUATIONS:
     ...
     +             NPTQ,QWORK,EPS,1,UARY,VARY,DLAM,IU)
     ...
     +              PHI1,NPTQ,QWORK,1,UARY,VARY,DLAM,IU)
   ...
   50     CONTINUE
      ELSE IF (INV.EQ.3) THEN
          EPS = 1.D-6
          CALL EVALINVERSEMAP(50,200,M,N,U,C,W0,W1,Z0,Z1,ALFA0,ALFA1,
     +         PHI0,PHI1,NPTQ,QWORK,EPS,IOPT,UARY,VARY,DLAM,IU)
      END IF
C
      PRINT '(/)'
   60 WRITE (6,FMT=*)
     +  'WANT TO TRY ANOTHER EXAMPLE? (1 FOR "YES",2 FOR "NO")'
      ...
      SUBROUTINE EVALINVERSEMAP(NR,NA,M,N,U,C,W0,W1,Z0,Z1,ALFA0,ALFA1,
     +     PHI0,PHI1,NPTQ,QWORK,EPS,IOPT,UARY,VARY,DLAM,IU)
...
C     .. Intrinsic Functions ..
      INTRINSIC ABS,ACOS,CEXP,CMPLX,CONJG,REAL

      INTEGER, INTENT(IN) :: NR,NA,M,N,IOPT,NPTQ,IU
      DOUBLE PRECISION, INTENT(IN) :: EPS, U
      DOUBLE COMPLEX, INTENT(IN) :: C

C     .. Input Arrays ..
      DOUBLE COMPLEX, INTENT(IN) :: W0(M),W1(N),Z0(M),Z1(N)
      DOUBLE PRECISION, INTENT(IN) :: ALFA0(M),ALFA1(M),PHI0(N),
     +     PHI1(N),QWORK(NPTQ*(3+2*(M+N))),UARY(8),VARY(3),DLAM

C     .. Local Params/Arrays ..
      DOUBLE PRECISION :: PI
      DOUBLE COMPLEX WPLOT(NR,NA), ZPLOT(NR,NA)
      DOUBLE PRECISION WNORM(NR,NA), RADII(NR), ARGUMENTS(NA)

      PI = ACOS(-1.D0)
      DO I = 1,NR
         RADII(I) = (1-U)*I/NR+U
      END DO
      RADII(NR) = RADII(NR) - (1E-5)
      DO I = 1,NA
         ARGUMENTS(I) = 2*PI*I/NA
      END DO
      DO I = 1,NR
         DO J = 1,NA
     ...
     +           ALFA1,PHI0,PHI1,NPTQ,QWORK,1,UARY,VARY,DLAM,IU)
         END DO
      END DO
      
      OPEN(100,FORM='unformatted',FILE='wplot.bin',ACTION='write')
      OPEN(101,FORM='unformatted',FILE='wnorm.bin',ACTION='write')
      OPEN(102,FORM='unformatted',FILE='zplot.bin',ACTION='write')
      DO I = 1,NR
         DO J = 1,NA
            WRITE(100) WPLOT(I,J)
            WRITE(101) WNORM(I,J)
      ...
      OPEN(104,FORM='unformatted',FILE='z0.bin',ACTION='write')
      DO I=1,M
         WRITE(104) Z0(I)
      END DO
      CLOSE(104)

      OPEN(105,FORM='unformatted',FILE='z1.bin',ACTION='write')
      DO I=1,N
         WRITE(105) Z1(I)
      END DO
      CLOSE(105)
